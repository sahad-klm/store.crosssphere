  ...
  return (
    ...
    >
      ...
      {browseAllCategory && !navHide && (
        <div className="absolute top-[76px] left-0 z-40 grid w-max grid-cols-2 gap-4 rounded-2xl border-[1.5px] border-[#0062b0] border-opacity-30 bg-white p-[30px]  drop-shadow-lg ">
          {BrowseAllCategories?.map((item) => (
            <Link
              aria-label="Read more"
              href={`/collections/${item.slug}`}
              key={item.id}
              className="group grid w-full grid-flow-col place-items-center gap-[14px] rounded border border-gray-300 border-opacity-20 py-2 px-4 hover:border-[#00539f] hover:opacity-70 hover:shadow-sm"
            >
              {item.icon}

              <p className="font-head text-[15px] leading-4 text-[#253d4e] group-hover:text-[#0062b0] ">
                {item.name}
              </p>
            </Link>
          ))}
