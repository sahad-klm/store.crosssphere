...
const FooterProducts = (): JSX.Element | any => {
  ...
  const popular = productsData.filter((item) => item.partOf === 'popular' && item)
  const tranding = productsData.filter((item) => item.partOf === 'trending' && item)
  const top_rated = productsData.filter((item) => item.partOf === 'top-rated' && item)
   const recent = [...productsData].sort((a) => a.id >= 103 ? 1 : -1)


  return (
    <section className="mt-[50px] flex w-full flex-col justify-center overflow-hidden px-4  md:items-center pb-10">
