    ...
    <div className="sm:grid w-full mx-4 mt-[44px]  gap-5 lg:grid-cols-3 flex flex-col sm:grid-cols-2 justify-between h-[246px]">
      {fracturedMainProduct?.map((item ) => (
        ...
        animate={{
          ...
          key={item.id}
          className={`bg-${item.bg}-100 flex relative p-10 rounded-lg flex-col justify-start w-full sm:max-w-lg group/edit`}
        >
          ...
          />
          <h4 className="font-head md:text-h4 text-[22px] text-[#253d4e] max-w-[250px] z-[1] capitalize group-hover/edit:-translate-y-1 duration-500 transition-all">
            {item.title}
          </h4>
